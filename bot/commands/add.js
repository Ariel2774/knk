module.exports = {
    ...
    run: async function(client, message, args) {
        if (!message.channel.name.includes("form-")) {
            message.channel.send({
                embed: {
                    title: `**❌ | Error**`,
                    description: `This is not a Form channel`,
                    color: 0xFF0000
                }
        ...
        if (!args[0]) {
            message.channel.send({
                embed: {
                    title: `**❌ | Error**`,
                    description: `you have to specify the role/member you wont to make him join's the Form!`,
                    color: 0xFF0000
                }
            }).then(async function(msg) {
                setTimeout(() => {
        ...
        message.channel.overwritePermissions([{
            ...
            message.channel.send({
                embed: {
                    title: '✅ | Done',
                    description: `${txt} has been added to this Form`,
                    color: 0x00D700
                }
            }).then(async function(msg) {
                setTimeout(() => {
