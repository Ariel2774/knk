...
module.exports = async function(client, button) {
        try {
            ...
            if (button.id == 'createTicket') {
                var nameer = `form-${button.clicker.user.username}`
                var checkTickets = button.guild.channels.cache.find(c => c.name == nameer.split('LAMARAN').join('-').toLocaleLowerCase());
                if (checkTickets) {
                    button.channel.send({
                        embed: {
                            color: 0xFF0000,
                            title: `**❌ | Error**`,
                            description: `You already have a Form open before`
                        }
                ...
                button.guild.channels.create(`form-${button.clicker.user.username}`, {
                    ...
                    channel.send(`<@${button.clicker.user.id}>`, {
                        embed: {
                            description: `Silahkan Kirim Lamaran Kalian Dengan Format Yang Sudah Kami Cantumkan\n\n\ Note: Jangan Delete/Close Form ,Jika Kamu Delete/Close Artinya Kamu Menggugurkan Diri`,
                            color: 0x2F3136
                        },
                        component: row
                    });
                ...
                if (!button.channel.name.includes("form-")) {
                    return;
                }
                ...
                button.channel.send({
                    embed: {
                        description: `Form has been Closed By <@!${button.clicker.user.id}>`,
                        color: 0xFFE700
                    }
                }).then(async function(m) {
                    setTimeout(() => {
                ...
                button.channel.send({
                    embed: {
                        description: 'Press **"⛔"** to delete the Form\nPress **"🔓"** to reopen the Form',
                        color: 0xFF0000
                    },
                    component: row
                }).then(async function(m) {
                ...
                button.channel.send({
                    embed: {
                        description: 'Form will be deleted in a few seconds',
                        color: 0xFF0000
                    }
                });
                setTimeout(() => {
                ...
                button.channel.send({
                    embed: {
                        description: `Form has been reopened By <@!${button.clicker.user.id}>`,
                        color: 0xFFE700
                    }
                }).then(async function(m) {
                    setTimeout(() => {
                ...
                button.guild.channels.create(`ticket-${button.clicker.user.username}`, {
                    ...
                    Press **"🔒"** to close this Form\n\n Jika Kamu Close/Delete Form Dinyatakan Gugur/Didiskualifikasi`,
                            color: 0x2F3136
                        },
                        component: row
                    });
                ...
                var msg = require('quick.db').fetch(`DeleteRenameMessage_${button.channel.id}`);
                button.channel.messages.fetch(msg).then(message => message.delete()).catch(err => { return });
                button.channel.setName('form-' + require('quick.db').fetch(`RenameTicket_${button.channel.id}`));
                button.channel.send({
                            embed: {
                                title: '✅',
                                description: `this Form name has been renamed to \`${require('quick.db').fetch(`RenameTicket_${button.channel.id}`)}\``,
                    color: 0x00D700
                }
            })
            require('quick.db').delete(`DeleteRenameMessage_${button.channel.id}`);
